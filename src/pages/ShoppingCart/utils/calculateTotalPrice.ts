...
export const calculateTotalPrice = (
  ...
  const prices = cartProducts?.map((cartProduct) => {
    ...
    if (isNaN(basePrice) || isNaN(quantity)) {
      Notify.create({
        message: `Ocurrió un error calculando el precio de ${cartProduct.product.name}, por lo que no se ha incluido en la cuenta, por favor intenta removerlo de tu carrito e incluyéndolo nuevamente.`,
        color: 'negative',
      });
      return 0;
    }
